class LoginChecksController < ApplicationController
    ...
    private
        ...
        def render_bad_request_error
            render json: BadRequestErrorSerializer.new(object: @login_check).serializable_hash,
                         status: 400
        end

        def render_successful_login_check
            render json: GeneralSuccessfulActionSerializer.new(action: "Login check").serializable_hash,
                         status: 200
        end
